...
$order_p_query=mysqli_query($db,"SELECT COUNT(*) AS num_of_orders, o.order_id, u.first_name, u.last_name, ca.contractor_id, ca.order_id, ca.amount as c_amount FROM orders AS o LEFT JOIN users AS u ON u.id=o.user_id LEFT JOIN contractor_orders AS ca ON o.order_id=ca.order_id WHERE o.status!='partial' AND o.is_payment_sent='0' AND o.is_trash='0' AND ca.contractor_id='".$admin_l_id."' ".$filter_by);
