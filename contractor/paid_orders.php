...
if(isset($_POST['search'])) {
	$_SESSION['paid_o_filter_data'] = array('filter_by'=>$post['filter_by'],'from_date'=>$post['from_date'],'to_date'=>$post['to_date'],'status'=>$post['status'],'payment_paid_batch_id'=>$post['payment_paid_batch_id']);
	setRedirect(CONTRACTOR_URL.'paid_orders.php');
}

if(isset($_GET['clear'])) {
	unset($_SESSION['paid_o_filter_data']);
	setRedirect(CONTRACTOR_URL.'paid_orders.php');
}

if(isset($_SESSION['paid_o_filter_data'])) {
	$model_filter_data = $_SESSION['paid_o_filter_data'];
...
$order_p_query=mysqli_query($db,"SELECT COUNT(*) AS num_of_orders, o.order_id, u.first_name, u.last_name, ca.contractor_id, ca.order_id, ca.amount as c_amount FROM orders AS o LEFT JOIN users AS u ON u.id=o.user_id LEFT JOIN contractor_orders AS ca ON o.order_id=ca.order_id WHERE o.status!='partial' AND o.is_payment_sent='1' AND o.is_trash='0' AND ca.contractor_id='".$admin_l_id."' ".$filter_by);
$order_p_data = mysqli_fetch_assoc($order_p_query);
$pages->set_total($order_p_data['num_of_orders']);

//Fetch list of order
$order_query=mysqli_query($db,"SELECT o.*, u.first_name, u.last_name, p.shop_name as aflt_shop_name, os.name AS order_status_name, ca.contractor_id, ca.order_id, ca.amount as c_amount FROM orders AS o LEFT JOIN users AS u ON u.id=o.user_id LEFT JOIN affiliate AS p ON p.id=o.affiliate_id LEFT JOIN order_status AS os ON os.id=o.status LEFT JOIN contractor_orders AS ca ON o.order_id=ca.order_id WHERE o.status!='partial' AND o.is_payment_sent='1' AND o.is_trash='0' AND ca.contractor_id='".$admin_l_id."' ".$filter_by." ".$order_by." ".$pages->get_limit());

$order_paid_batch_q=mysqli_query($db,"SELECT o.*, u.first_name, u.last_name, ca.contractor_id, ca.order_id, ca.amount as c_amount FROM orders AS o LEFT JOIN users AS u ON u.id=o.user_id LEFT JOIN contractor_orders AS ca ON o.order_id=ca.order_id WHERE o.status!='partial' AND o.is_payment_sent='1' AND o.is_trash='0' AND ca.contractor_id='".$admin_l_id."' GROUP BY o.payment_paid_batch_id ORDER BY o.date DESC");
