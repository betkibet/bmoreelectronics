...
function get_model_data_list($device_id, $devices_id, $cat_id) {
	...
	$model_query=mysqli_query($db,"SELECT m.*, d.title AS device_title, d.sef_url AS device_sef_url, b.title AS brand_title FROM mobile AS m LEFT JOIN devices AS d ON d.id=m.device_id LEFT JOIN brand AS b ON b.id=m.brand_id WHERE m.published=1 ".$sql_whr." ORDER BY m.ordering DESC");
...
function get_single_model_data($req_model_id) {
	global $db;
	$response = array();
	$query=mysqli_query($db,"SELECT m.*, d.title AS device_title, d.sef_url AS device_sef_url, b.sef_url AS brand_sef_url, b.title AS brand_title, c.fields_type AS fields_cat_type FROM mobile AS m LEFT JOIN devices AS d ON d.id=m.device_id LEFT JOIN brand AS b ON b.id=m.brand_id LEFT JOIN categories AS c ON c.id=m.cat_id WHERE m.id='".$req_model_id."'");
