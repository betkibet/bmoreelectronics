...
$user_p_query=mysqli_query($db,"SELECT COUNT(*) AS num_of_partners FROM partners WHERE 1 ".$filter_by." ORDER BY id DESC");
$user_p_data = mysqli_fetch_assoc($user_p_query);
$pages->set_total($user_p_data['num_of_partners']);

//Fetch user list
$user_query=mysqli_query($db,"SELECT * FROM partners WHERE 1 ".$filter_by." ORDER BY id DESC ".$pages->get_limit()."");

//Template file
require_once("views/partner/partners.php");
