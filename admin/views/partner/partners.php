          ...
          <div class="m-portlet__head">
            ...
            <div class="m-portlet__head-tools">
              <ul class="m-portlet__nav">
                  ...
                  <a href="edit_partner.php" class="btn btn-accent m-btn m-btn--custom m-btn--pill m-btn--icon m-btn--air">
                    <span>
                ...
                <?php /*?><li class="m-portlet__nav-item">
                  <form action="controllers/partner.php" method="POST">
                    ...
                    <form method="post">
                      <div class="input-group">
                        ...
                        <?php	if($_REQUEST['filter_by'])
        									echo '<a href="partners.php"><button class="btn btn-danger m-btn m-btn--custom m-btn--pill m-btn--icon m-btn--air btn-xs float-right" type="button">Clear</button></a>';
                        ?>
                      </div>
                    </form>
                  </label>
                  ...
                  <table class="table table-bordered table-hover table-checkable dataTable dtr-inline">
                    ...
                    <tbody>
                      ...
                      <tr>
						...
						<td><a href="<?=SITE_URL?>apr/?apr=<?=$customer_data['store_name']?>" target="_blank"><?=$customer_data['store_name']?></a></td>
						<td><?=$customer_data['name']?></td>
						<td><?=$customer_data['email']?></td>
						<td Width="190">
	  					<?php
						if($customer_data['status']==1) {
							echo '<a href="controllers/partner.php?p_id='.$customer_data['id'].'&status=0"><button class="btn btn-brand m-btn m-btn--custom m-btn--pill m-btn--icon m-btn--air btn-xs" style="pointer-events: none;">Active</button></a>';
						} elseif($customer_data['status']==0) {
							echo '<a href="controllers/partner.php?p_id='.$customer_data['id'].'&status=1"><button class="btn btn-danger m-btn m-btn--custom m-btn--pill m-btn--icon m-btn--air btn-xs" style="pointer-events: none;">Inactive</button></a>';
						} ?>
                        <a href="edit_partner.php?id=<?=$customer_data['id']?>" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill"><i class="la la-edit"></i></a>
      					<a href="controllers/partner.php?d_id=<?=$customer_data['id']?>" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" onclick="return confirm('are you sure to delete this record?')"><i class="la la-trash"></i></a>
                        </td>
                      </tr>
                      <?php }
                      } ?>
