                  ...
                  <div id="m_table_1_filter" class="dataTables_filter float-left">
                    <form method="post">
                        <div class="input-group">
                          ...
                          <?php
						  if($post['filter_by']!="") {
          					echo '<a href="system_page.php?clear" class="btn btn-alt btn-danger ml-2">Clear <i class="la la-remove"></i></a>';
						  } ?>
                        </div>
                    </form>
                  </div>
                      ...
                      <tr>
                        ...
                        <td Width="190">
							...
							if($page_data['published']==1) {
								echo '<a href="controllers/system_page.php?p_id='.$page_data['id'].'&published=0"><button class="btn btn-brand m-btn m-btn--custom m-btn--pill m-btn--icon m-btn--air btn-xs" style="pointer-events: none;">Published</button></a>';
							} elseif($page_data['published']==0) {
								echo '<a href="controllers/system_page.php?p_id='.$page_data['id'].'&published=1"><button class="btn btn-danger m-btn m-btn--custom m-btn--pill m-btn--icon m-btn--air btn-xs" style="pointer-events: none;">Unpublished</button></a>';
							}
							
							if($prms_page_edit == '1') { ?>
                          		<a href="edit_system_page.php?id=<?=$page_data['id']?>&slug=<?=$page_data['slug']?>" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill"><i class="la la-edit"></i></a>

                            <?php
						    }
                          /*if(!in_array($page_data['slug'],$inbuild_page_slug)) { ?>
                            <a href="controllers/system_page.php?d_id=<?=$page_data['id']?>" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" onclick="return confirm('are you sure to delete this record?')"><i class="la la-trash"></i></a>
						<?php
						}*/
						?>
                        </td>
