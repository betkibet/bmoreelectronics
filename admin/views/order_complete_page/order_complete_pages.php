            ...
            <div class="m-portlet__head-tools">
              <ul class="m-portlet__nav">
                ...
                <li class="m-portlet__nav-item">
                  <a href="edit_order_complete_page.php" class="btn btn-accent m-btn m-btn--custom m-btn--pill m-btn--icon m-btn--air">
                    <span>
          ...
          <div class="m-portlet__body">
            ...
            <div id="m_table_1_wrapper" class="dataTables_wrapper container-fluid dt-bootstrap4 no-footer">
                  ...
                  <div id="m_table_1_filter" class="dataTables_filter float-left">
                    <form method="post">
                        <div class="input-group">
                          ...
                          <?php
						  if($post['filter_by']!="") {
          					echo '<a href="order_complete_pages.php?clear" class="btn btn-alt btn-danger ml-2">Clear <i class="la la-remove"></i></a>';
						  } ?>
                        </div>
                    </form>
                  </div>
			  ...
			  <div class="row m--margin-top-20">
				<div class="col-sm-12">
					<form action="controllers/order_complete_page.php" method="POST">
              ...
              <div class="row">
                <div class="col-sm-12">
                  <form action="controllers/order_complete_pages.php" method="post">
                    <table class="table table-bordered table-hover table-checkable dataTable no-footer dtr-inline">
                      ...
                      <tbody>
						...
						if($num_rows>0) {
						  while($order_complete_data=mysqli_fetch_assoc($query)) {
						    $tooltips_content = "";
							if($order_complete_data['id'] == '1') {
								$tooltips_content = $order_complete_mail_in_tooltips_admin;
							} elseif($order_complete_data['id'] == '2') {
								$tooltips_content = $order_complete_meet_up_tooltips_admin;
							} elseif($order_complete_data['id'] == '3') {
								$tooltips_content = $order_complete_drop_by_tooltips_admin;
							} ?>
							<tr>
							  <td>
								<label class="m-checkbox m-checkbox--single m-checkbox--solid m-checkbox--brand">
								  <input type="checkbox" onclick="clickontoggle('<?=$order_complete_data['id']?>');" class="sub_chk m-input" name="chk[]" value="<?=$order_complete_data['id']?>">
								  <span></span>
								</label>
							  </td>
							  <td>
								<?=$order_complete_data['title']?>&nbsp;<a href="javascript:void(0);" data-toggle="tooltip" data-skin="light" title="<?=$tooltips_content?>" data-html="true"><span class="fa fa-info-circle"></span></a>
							  </td>
							  <?php /*?><td>
								<input type="text" class="m-input--square form-control m-input" id="ordering<?=$order_complete_data['id']?>" value="<?=$order_complete_data['ordering']?>" name="ordering[<?=$order_complete_data['id']?>]">
							  </td><?php */?>
							  <td Width="120">
								<?php
								/*if($order_complete_data['type']!="fixed") {
									if($order_complete_data['status']==1) {
										echo '<a href="controllers/order_complete_page.php?p_id='.$order_complete_data['id'].'&published=0"><button class="btn btn-brand m-btn m-btn--custom m-btn--pill m-btn--icon m-btn--air btn-xs" style="pointer-events: none;">Published</button></a>';
									} elseif($order_complete_data['status']==0) {
										echo '<a href="controllers/order_complete_page.php?p_id='.$order_complete_data['id'].'&published=1"><button class="btn btn-danger m-btn m-btn--custom m-btn--pill m-btn--icon m-btn--air btn-xs" style="pointer-events: none;">Unpublished</button></a>';
									}
								}*/
								
								if($prms_order_edit == '1') { ?>
								<a href="edit_order_complete_page.php?id=<?=$order_complete_data['id']?>" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill"><i class="la la-edit"></i></a>
								<?php
								}
								/*if($order_complete_data['type']!="fixed") {
									if($prms_order_delete == '1') { ?>
									<a href="controllers/order_complete_page.php?d_id=<?=$order_complete_data['id']?>" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" onclick="return confirm('are you sure to delete this record?')"><i class="la la-trash"></i></a>
									<?php
									}
								}*/ ?>
							  </td>
