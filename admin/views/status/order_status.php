            ...
            <div class="m-portlet__head-tools">
              <ul class="m-portlet__nav">
                ...
                <li class="m-portlet__nav-item">
                  <a href="edit_order_status.php" class="btn btn-accent m-btn m-btn--custom m-btn--pill m-btn--icon m-btn--air">
                    <span>
          ...
          <div class="m-portlet__body">
            ...
            <div id="m_table_1_wrapper" class="dataTables_wrapper container-fluid dt-bootstrap4 no-footer">
                  ...
                  <div id="m_table_1_filter" class="dataTables_filter float-left">
                    <form method="post">
                        <div class="input-group">
                          ...
                          <?php
						  if($post['filter_by']!="") {
          					echo '<a href="order_status.php?clear" class="btn btn-alt btn-danger ml-2">Clear <i class="la la-remove"></i></a>';
						  } ?>
                        </div>
                    </form>
                  </div>
			  ...
			  <div class="row m--margin-top-20">
				<div class="col-sm-12">
					<form action="controllers/order_status.php" method="POST">
                          ...
                          <td Width="220">
							...
							if($order_status_data['type']!="fixed") {
								if($order_status_data['status']==1) {
									echo '<a href="controllers/order_status.php?p_id='.$order_status_data['id'].'&published=0"><button class="btn btn-brand m-btn m-btn--custom m-btn--pill m-btn--icon m-btn--air btn-xs" style="pointer-events: none;">Published</button></a>';
								} elseif($order_status_data['status']==0) {
									echo '<a href="controllers/order_status.php?p_id='.$order_status_data['id'].'&published=1"><button class="btn btn-danger m-btn m-btn--custom m-btn--pill m-btn--icon m-btn--air btn-xs" style="pointer-events: none;">Unpublished</button></a>';
								}
							}
							
							if($prms_order_edit == '1') { ?>
                            <a href="edit_order_status.php?id=<?=$order_status_data['id']?>" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill"><i class="la la-edit"></i></a>
							<?php
							}
							if($order_status_data['type']!="fixed") {
								if($prms_order_delete == '1') { ?>
								<a href="controllers/order_status.php?d_id=<?=$order_status_data['id']?>" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" onclick="return confirm('are you sure to delete this record?')"><i class="la la-trash"></i></a>
								<?php
								}
							} ?>
                          </td>
