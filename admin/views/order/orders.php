			...
			<div class="m-portlet__head-tools">
              <ul class="m-portlet__nav">
                ...
                <li class="m-portlet__nav-item">
                  <a href="add_order.php?order_mode=unpaid" class="btn btn-accent m-btn m-btn--custom m-btn--pill m-btn--icon m-btn--air">
                    <span>
                  ...
                  <div id="m_table_1_filter" class="dataTables_filter float-left">
                    <form method="post">
                        <div class="form-group row">
							...
							<div class="col-lg-3">
						  		<input type="text" class="form-control form-control-md ml-2 datepicker" placeholder="From Date" name="from_date" id="from_date" value="<?=_dt_parse($post['from_date'])?>" autocomplete="nope">
						  	</div>
							<div class="col-lg-3">
						  		<input type="text" class="form-control form-control-md ml-2 datepicker" placeholder="To Date" name="to_date" id="to_date" value="<?=_dt_parse($post['to_date'])?>" autocomplete="nope">
						  	</div>
							<div class="col-lg-3">
						...
						<div class="form-group row">
							<div class="col-lg-4">	
							  ...
							  if($post['filter_by']!="" || $post['from_date']!="" || $post['to_date']!="" || $post['status']!="") {
								echo '<a href="orders.php?clear" class="btn btn-alt btn-danger ml-2">Clear <i class="la la-remove"></i></a>';
							  } ?>
							</div>
						</div>
			  ...
			  <div class="row m--margin-top-20">
				<div class="col-sm-12">
					<form action="controllers/order/order.php" method="POST">
						...
						<button class="btn btn-sm btn-danger m-btn m-btn--custom m-btn--pill m-btn--icon m-btn--air bulk_archive" name="bulk_archive">Bulk Archive</button>
						<?php
						} ?>
						<button class="btn btn-sm btn-success m-btn m-btn--custom m-btn--pill m-btn--icon m-btn--air bulk_set_paid" name="bulk_set_paid">Bulk Paid</button>
					</form>
				</div>
			  </div>
                    ...
                    <table class="table table-bordered table-hover table-checkable dataTable dtr-inline" <?php /*?>id="m_table_1"<?php */?>>
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td><a href="edit_order.php?order_id=<?=$order_data['order_id']?>&order_mode=unpaid"><?=$order_data['order_id']?></a></td>
                          <td><a href="edit_user.php?id=<?=$order_data['user_id']?>"><?=$order_data['first_name'].' '.$order_data['last_name']?></a></td>
						  <td><a href="edit_contractor.php?id=<?=$order_data['contractor_id']?>"><?=$order_data['contractor_name']?></a></td>
						  <td>
						  <?php
						  if($order_data['aflt_shop_name']!="") { ?>
						  <a href="edit_affiliate.php?id=<?=$order_data['affiliate_id']?>"><?=$order_data['aflt_shop_name']?></a>
						  <?php
						  } ?>
						  </td>
                          ...
                          <td Width="150">
                            <?php /*?><a href="order_offer.php?order_id=<?=$order_data['order_id']?>&order_mode=unpaid" class="btn btn-accent m-btn m-btn--custom m-btn--pill m-btn--icon m-btn--air btn-xs">Offer</a><?php */?>
							<?php
							if($prms_order_edit == '1') { ?>
                            <a href="edit_order.php?order_id=<?=$order_data['order_id']?>&order_mode=unpaid" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill"><i class="la la-edit"></i></a>
							<?php
							}
							if($prms_order_delete == '1') { ?>
                            <a href="controllers/order/order.php?a_id=<?=$order_data['order_id']?>&order_mode=unpaid" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" onclick="return confirm('Are you sure you want to archive this record?')"><i class="la la-trash"></i></a>
							<?php
							} ?>
                          </td>
                        </tr>
