...
if($post['order_mode'] == "paid") {
	...
	$back_url = ADMIN_URL.'archive_orders.php';
} elseif($post['order_mode'] == "abandone") {
	$back_url = ADMIN_URL.'abandone_orders.php';
} else {
					...
					<div class="m-portlet__head m-portlet__head-bg-brand">
						...
						<div class="m-portlet__head-tools">
						  <ul class="m-portlet__nav">
							<li class="m-portlet__nav-item"><a class="btn btn-success" href="javascript:open_window('<?=SITE_URL?>admin/views/print/print_delivery_note.php?order_id=<?=$order_id?>&access_token=<?=$o_access_token?>');">Delivery note <i class="fa fa-print"></i></a></li>
							...
							<li class="m-portlet__nav-item"><a class="btn btn-alt btn-success" href="javascript:open_window('<?=SITE_URL?>admin/views/print/sales_confirmation.php?order_id=<?=$order_id?>&access_token=<?=$o_access_token?>');">sales Confirmation <i class="fa fa-download"></i></a></li>
							<?php
										...
										<div class="m-section">
											<div class="m-section__content">
												...
												<dl class="no-bottom-margin">
												  ...
												  <?php
												  if($order_data_before_saved['location_name']) { ?>
												  <br />
												  <dd><strong>Shipping Method:</strong> <?=$order_data_before_saved['sales_pack']?></dd>
												  <dd><strong>Store Location:</strong> <?=$order_data_before_saved['location_name']?></dd>
												  <dd><strong>Store Date:</strong> <?=format_date($order_data_before_saved['store_date'])?></dd>
												  <dd><strong>Store Time:</strong> <?=format_time($order_data_before_saved['store_time'])?></dd>
												  <?php
												  } ?>
												</dl>
												</div>
											  ...
											  <dd>
												...
												<div class="form-group m-form__group row">
													<div class="col-md-12" style="padding-right:5px;">
													  <select name="shipping_method" id="shipping_method" class="form-control-sm" style="width:150px;">
														 <option value=""> - Select - </option>
														 <option value="post_me_a_prepaid_label" <?php if($sales_pack=='post_me_a_prepaid_label'){echo 'selected="selected"';}?>><?=$cart_shipping_method_post_me_a_prepaid_label_title?></option>
														 <option value="print_a_prepaid_label" <?php if($sales_pack=='print_a_prepaid_label'){echo 'selected="selected"';}?>><?=$cart_shipping_method_print_a_prepaid_label_title?></option>
														 <option value="use_my_own_courier" <?php if($sales_pack=='use_my_own_courier'){echo 'selected="selected"';}?>><?=$cart_shipping_method_use_my_own_courier_title?></option>
														 <option value="we_come_for_you" <?php if($sales_pack=='we_come_for_you'){echo 'selected="selected"';}?>><?=$cart_shipping_method_we_come_for_you_title?></option>
														 <option value="store" <?php if($sales_pack=='store'){echo 'selected="selected"';}?>><?=$cart_shipping_method_store_title?></option>
														 <option value="starbucks" <?php if($sales_pack=='starbucks'){echo 'selected="selected"';}?>><?=$cart_shipping_method_starbucks_title?></option>
													  </select>
                      ...
                      <div class="col-md-12">
                        ...
                        <table class="table table-sm m-table m-table--head-bg-brand">
							<thead class="thead-inverse">
								...
								<tr>
									<?php /*?><th width="25">#</th><?php */?>
									<?php /*?><th width="8%">Item ID</th>
								...
								foreach($order_item_list as $order_item_list_data) {
									...
									<tr>
										...
										<div class="form-group m-form__group">
											...
											<button type="button" class="btn btn-primary btn-sm" onClick="AddItemMedia(<?=$order_item_list_data['id']?>);return false;">Add Media</button>&nbsp;<button type="button" class="btn btn-primary btn-sm" onClick="CheckDevice(<?=$order_item_list_data['id']?>,'','');return false;">Check Device</button>
											<?php
											if(!empty($order_item_testing_data)) { ?>
											&nbsp;<button type="button" class="btn btn-primary btn-sm mt-2" onClick="CheckDevice(<?=$order_item_list_data['id']?>,'checked','<?=_dt_parse($order_item_list_data['device_title'].($order_item_list_data['device_title']!="" && $order_item_list_data['model_title']!=""?' - ':'').$order_item_list_data['model_title'])?>');return false;">Checked</button>
											<?php
											} ?>
										</div>
										<?php
										...
										<td>
											<input type="number" class="form-control m-input item_price" id="price[]" name="price[<?=$item_id?>]" value="<?=$order_item_list_data['price']?>">
										    <?php /*?><?=amount_fomat($order_item_list_data['price'])?><?php */?>
										</td>
										<td>
			...
			<div class="col-md-12">
				<div class="m-portlet m-portlet--tab">
					...
					<div class="m-portlet__body m--padding-top-10 m--padding-bottom-10">
						...
						<div class="form-group m-form__group">
							<a class="btn btn-md btn-primary" data-toggle="modal" href="#AssignToContractor">Assign to Contractor</a>
							<?php
...
<div class="modal fade" id="AssignToContractor" tabindex="-1" role="dialog" aria-labelledby="AssignToContractorLabel" aria-hidden="true">
	<div class="modal-dialog modal-md" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="AssignToContractorLabel">Assign to contractor</h5>
					...
					<select required="required" id="contractor_id" name="contractor_id" class="form-control m-input">
						  ...
						  if($contr_num_of_rows>0) {
							  foreach($contractor_data_list as $contractor_data) { ?>
								<option value="<?=$contractor_data['id']?>" <?php if($contractor_data['id'] == $assigned_contractor_data['contractor_id']) {echo 'selected="selected"';}?>><?=$contractor_data['name'].($contractor_data['company_name']?' ('.$contractor_data['company_name'].')':'')?></option>
							  <?php
							  }
						  } ?>
					</select>
			...
			<div class="modal-body">
				<div id="payment_box">
					...
					if($choosed_payment_option['bank']=="bank") { ?>
						<div class="head_row bank_head pmnt_bank_opt <?php if($default_payment_option=="bank"){echo 'active';}?> clearfix"><span class="round_box">&nbsp;</span><span class="icon"></span><span class="title"><?=$payment_method_bank_heading_text?></span></div>
						<div class="detail_row bank_detail clearfix <?php if($default_payment_option=="bank"){echo 'active';}?>">
							<div class="inner_box row">
								<div class="form_group col-sm-4">
									<label>Account Holder Name</label>
									<input type="text" class="textbox" id="act_name" name="act_name" placeholder="" value="<?=(!empty($payment_method_details['account_holder_name'])?$payment_method_details['account_holder_name']:'')?>" autocomplete="nope">
								</div>
								<div class="form_group col-sm-4">
									<label>Account Number</label>
									<input type="text" class="textbox" id="act_number" name="act_number" placeholder="" value="<?=(!empty($payment_method_details['account_number'])?$payment_method_details['account_number']:'')?>" autocomplete="nope">
								</div>
								<div class="form_group col-sm-4">
									<label>Short Code</label>
									<input type="text" class="textbox" id="act_short_code" name="act_short_code" placeholder="" value="<?=(!empty($payment_method_details['short_code'])?$payment_method_details['short_code']:'')?>" autocomplete="nope">
								</div>
							</div>
						</div>
					<?php
					...
					if($choosed_payment_option['paypal']=="paypal") { ?>
						<div class="head_row paypal_head pmnt_paypal_opt <?php if($default_payment_option=="paypal"){echo 'active';}?>">
							<span class="round_box">&nbsp;</span>
							<span class="icon">i</span>
							<span class="title"><?=$payment_method_paypal_heading_text?></span>
						...
						<div class="detail_row paypal_detail clearfix <?php if($default_payment_option=="paypal"){echo 'active';}?>">
							<div class="inner_box row">
								<div class="form_group col-sm-6">
									...
									<input type="text" class="textbox" placeholder="" id="paypal_address"  name="paypal_address" value="<?=(!empty($payment_method_details['paypal_address'])?$payment_method_details['paypal_address']:'')?>" autocomplete="nope">
								</div>
								<div class="form_group col-sm-6">
									<label>Confirm PayPal Address</label>
									<input type="text" class="textbox" placeholder="" id="confirm_paypal_address"  name="confirm_paypal_address" value="<?=(!empty($payment_method_details['paypal_address'])?$payment_method_details['paypal_address']:'')?>" autocomplete="nope">
								</div>
							</div>
						</div>
					<?php
					...
					if($choosed_payment_option['venmo']=="venmo") { ?>
						<div class="head_row venmo_head pmnt_venmo_opt <?php if($default_payment_option=="venmo"){echo 'active';}?>">
							<span class="round_box">&nbsp;</span>
							<span class="icon">i</span>
							<span class="title"><?=$payment_method_venmo_heading_text?></span>
						...
						<div class="detail_row venmo_detail clearfix <?php if($default_payment_option=="venmo"){echo 'active';}?>">
							<div class="inner_box row">
								<div class="form_group col-sm-12">
									...
									<input type="text" class="textbox" placeholder="" id="venmo_address"  name="venmo_address" value="<?=(!empty($payment_method_details['venmo_address'])?$payment_method_details['venmo_address']:'')?>" autocomplete="nope">
								</div>
							</div>
						</div>
					<?php
					...
					if($choosed_payment_option['zelle']=="zelle") { ?>
						<div class="head_row zelle_head pmnt_zelle_opt <?php if($default_payment_option=="zelle"){echo 'active';}?>">
							<span class="round_box">&nbsp;</span>
							<span class="icon">i</span>
							<span class="title"><?=$payment_method_zelle_heading_text?></span>
						...
						<div class="detail_row zelle_detail clearfix <?php if($default_payment_option=="zelle"){echo 'active';}?>">
							<div class="inner_box row">
								<div class="form_group col-sm-12">
									...
									<input type="text" class="textbox" placeholder="" id="zelle_address"  name="zelle_address" value="<?=(!empty($payment_method_details['zelle_address'])?$payment_method_details['zelle_address']:'')?>" autocomplete="nope">
								</div>
							</div>
						</div>
					<?php
					...
					if($choosed_payment_option['amazon_gcard']=="amazon_gcard") { ?>
						<div class="head_row amazon_gcard_head pmnt_amazon_gcard_opt <?php if($default_payment_option=="amazon_gcard"){echo 'active';}?>">
							<span class="round_box">&nbsp;</span>
							<span class="icon">i</span>
							<span class="title"><?=$payment_method_amazon_gcard_heading_text?></span>
							
						</div>
						<div class="detail_row amazon_gcard_detail clearfix <?php if($default_payment_option=="amazon_gcard"){echo 'active';}?>">
							<div class="inner_box row">
								<div class="form_group col-sm-12">
									<label>Amazon GCard Address</label>
									<input type="text" class="textbox" placeholder="" id="amazon_gcard_address"  name="amazon_gcard_address" value="<?=(!empty($payment_method_details['amazon_gcard_address'])?$payment_method_details['amazon_gcard_address']:'')?>" autocomplete="nope">
								</div>
							</div>
						</div>
					<?php
					...
					if($choosed_payment_option['cash']=="cash") { ?>
						<div class="head_row cash_head pmnt_cash_opt <?php if($default_payment_option=="cash"){echo 'active';}?> clearfix"><span class="round_box">&nbsp;</span><span class="icon"></span><span class="title"><?=$payment_method_cash_heading_text?></span></div>
						<div class="detail_row cash_detail clearfix <?php if($default_payment_option=="cash"){echo 'active';}?>">
							<div class="inner_box row">
								<div class="form_group col-sm-6">
									<label>Cash Name</label>
									<input type="text" class="textbox" id="cash_name" name="cash_name" placeholder="" value="<?=(!empty($payment_method_details['cash_name'])?$payment_method_details['cash_name']:'')?>" autocomplete="nope">
								</div>
								<div class="form_group col-sm-6">
									<label>Cash Phone</label>
									<input type="text" class="textbox" id="cash_phone" name="cash_phone" placeholder="" value="<?=(!empty($payment_method_details['cash_phone'])?$payment_method_details['cash_phone']:'')?>" autocomplete="nope">
								</div>
							</div>
						</div>
					<?php
					...
					if($choosed_payment_option['cash_app']=="cash_app") { ?>
						<div class="head_row cash_app_head pmnt_cash_app_opt <?php if($default_payment_option=="cash_app"){echo 'active';}?>">
							<span class="round_box">&nbsp;</span>
							<span class="icon">i</span>
							<span class="title"><?=$payment_method_cash_app_heading_text?></span>
							
						</div>
						<div class="detail_row cash_app_detail clearfix <?php if($default_payment_option=="cash_app"){echo 'active';}?>">
							<div class="inner_box row">
								<div class="form_group col-sm-12">
									<label>Cash App Address</label>
									<input type="text" class="textbox" placeholder="" id="cash_app_address"  name="cash_app_address" value="<?=(!empty($payment_method_details['cash_app_address'])?$payment_method_details['cash_app_address']:'')?>" autocomplete="nope">
								</div>
							</div>
						</div>
					<?php
					...
					if($choosed_payment_option['apple_pay']=="apple_pay") { ?>
						<div class="head_row apple_pay_head pmnt_apple_pay_opt <?php if($default_payment_option=="apple_pay"){echo 'active';}?>">
							<span class="round_box">&nbsp;</span>
							<span class="icon">i</span>
							<span class="title"><?=$payment_method_apple_pay_heading_text?></span>
							
						</div>
						<div class="detail_row apple_pay_detail clearfix <?php if($default_payment_option=="apple_pay"){echo 'active';}?>">
							<div class="inner_box row">
								<div class="form_group col-sm-12">
									<label>Apple Pay Address</label>
									<input type="text" class="textbox" placeholder="" id="apple_pay_address"  name="apple_pay_address" value="<?=(!empty($payment_method_details['apple_pay_address'])?$payment_method_details['apple_pay_address']:'')?>" autocomplete="nope">
								</div>
							</div>
						</div>
					<?php
					...
					if($choosed_payment_option['google_pay']=="google_pay") { ?>
						<div class="head_row google_pay_head pmnt_google_pay_opt <?php if($default_payment_option=="google_pay"){echo 'active';}?>">
							<span class="round_box">&nbsp;</span>
							<span class="icon">i</span>
							<span class="title"><?=$payment_method_google_pay_heading_text?></span>
						...
						<div class="detail_row google_pay_detail clearfix <?php if($default_payment_option=="google_pay"){echo 'active';}?>">
							<div class="inner_box row">
								<div class="form_group col-sm-12">
									...
									<input type="text" class="textbox" placeholder="" id="google_pay_address"  name="google_pay_address" value="<?=(!empty($payment_method_details['google_pay_address'])?$payment_method_details['google_pay_address']:'')?>" autocomplete="nope">
								</div>
							</div>
						</div>
					<?php
					...
					if($choosed_payment_option['coinbase']=="coinbase") { ?>
						<div class="head_row coinbase_head pmnt_coinbase_opt <?php if($default_payment_option=="coinbase"){echo 'active';}?>">
							<span class="round_box">&nbsp;</span>
							<span class="icon">i</span>
							<span class="title"><?=$payment_method_coinbase_heading_text?></span>
							
						</div>
						<div class="detail_row coinbase_detail clearfix <?php if($default_payment_option=="coinbase"){echo 'active';}?>">
							<div class="inner_box row">
								<div class="form_group col-sm-12">
									<label>Coinbase Address</label>
									<input type="text" class="textbox" placeholder="" id="coinbase_address"  name="coinbase_address" value="<?=(!empty($payment_method_details['coinbase_address'])?$payment_method_details['coinbase_address']:'')?>" autocomplete="nope">
								</div>
							</div>
						</div>
					<?php
					...
					if($choosed_payment_option['facebook_pay']=="facebook_pay") { ?>
						<div class="head_row facebook_pay_head pmnt_facebook_pay_opt <?php if($default_payment_option=="facebook_pay"){echo 'active';}?>">
							<span class="round_box">&nbsp;</span>
							<span class="icon">i</span>
							<span class="title"><?=$payment_method_facebook_pay_heading_text?></span>
							
						</div>
						<div class="detail_row facebook_pay_detail clearfix <?php if($default_payment_option=="facebook_pay"){echo 'active';}?>">
							<div class="inner_box row">
								<div class="form_group col-sm-12">
									<label>Facebook Pay Address</label>
									<input type="text" class="textbox" placeholder="" id="facebook_pay_address"  name="facebook_pay_address" value="<?=(!empty($payment_method_details['facebook_pay_address'])?$payment_method_details['facebook_pay_address']:'')?>" autocomplete="nope">
								</div>
							</div>
						</div>
					<?php
...
jQuery(document).ready(function ($) {
	...
	$(".<?=$payment_methods_nm_v?>_head").click(function(){
		$(this).addClass("active");
		$('.<?=$payment_methods_nm_v?>_detail').addClass("active");
		<?php
		if($payment_methods_nm_v!="cash") {
			echo '$(".cash_head").removeClass("active");';
			echo '$(".cash_detail").removeClass("active");';
		}
		if($payment_methods_nm_v!="paypal") {
			echo '$(".paypal_head").removeClass("active");';
			echo '$(".paypal_detail").removeClass("active");';
		}
		if($payment_methods_nm_v!="cheque") {
			echo '$(".cheque_head").removeClass("active");';
			echo '$(".cheque_detail").removeClass("active");';
		}
		if($payment_methods_nm_v!="venmo") {
			echo '$(".venmo_head").removeClass("active");';
			echo '$(".venmo_detail").removeClass("active");';
		}
		if($payment_methods_nm_v!="zelle") {
			echo '$(".zelle_head").removeClass("active");';
			echo '$(".zelle_detail").removeClass("active");';
		}
		if($payment_methods_nm_v!="amazon_gcard") {
			echo '$(".amazon_gcard_head").removeClass("active");';
			echo '$(".amazon_gcard_detail").removeClass("active");';
		}
		if($payment_methods_nm_v!="bank") {
			echo '$(".bank_head").removeClass("active");';
			echo '$(".bank_detail").removeClass("active");';
		}
		if($payment_methods_nm_v!="cash_app") {
			echo '$(".cash_app_head").removeClass("active");';
			echo '$(".cash_app_detail").removeClass("active");';
		}
		if($payment_methods_nm_v!="apple_pay") {
			echo '$(".apple_pay_head").removeClass("active");';
			echo '$(".apple_pay_detail").removeClass("active");';
		}
		if($payment_methods_nm_v!="google_pay") {
			echo '$(".google_pay_head").removeClass("active");';
			echo '$(".google_pay_detail").removeClass("active");';
		}
		if($payment_methods_nm_v!="coinbase") {
			echo '$(".coinbase_head").removeClass("active");';
			echo '$(".coinbase_detail").removeClass("active");';
		}
		if($payment_methods_nm_v!="facebook_pay") {
			echo '$(".facebook_pay_head").removeClass("active");';
			echo '$(".facebook_pay_detail").removeClass("active");';
		} ?>
	});
	
	$(".pmnt_<?=$payment_methods_nm_v?>_opt").click(function() {
		$("#payment_method").val('<?=$payment_methods_nm_v?>');
	});
	<?php
	} ?>
...
function check_pmt_method_form() {
	...
	if(payment_method=="amazon_gcard") {
		if(document.getElementById("amazon_gcard_address").value.trim()=="") {
...
function open_window(url) {
	window.open(url,"Loading",'toolbar=0,location=0,directories=0,status=0,menubar=0,scrollbars=1,resizable=0,width=800,height=800');
}
