			...
			<div class="m-portlet__head-tools">
              <ul class="m-portlet__nav">
                ...
                <li class="m-portlet__nav-item">
                  <a href="add_order.php?order_mode=paid" class="btn btn-accent m-btn m-btn--custom m-btn--pill m-btn--icon m-btn--air">
                    <span>
						...
						<div class="form-group row">
							<div class="col-lg-3">
								<select name="payment_paid_batch_id" id="payment_paid_batch_id" class="form-control m-input custom-select">
									...
									while($order_paid_batch_data=mysqli_fetch_assoc($order_paid_batch_q)) {
										if($order_paid_batch_data['payment_paid_batch_id']) { ?>
							...
							<div class="col-lg-4">
							  ...
							  if($post['filter_by']!="" || $post['from_date']!="" || $post['to_date']!="" || $post['status']!="") {
								echo '<a href="paid_orders.php?clear" class="btn btn-alt btn-danger ml-2">Clear <i class="la la-remove"></i></a>';
							  } ?>
							</div>
                        </div>
                    </form>
			  ...
			  <div class="row m--margin-top-20">
				<div class="col-sm-12">
					<form action="controllers/order/order.php" method="POST">
						...
						<input type="hidden" name="order_mode" id="order_mode" value="paid">
                    ...
                    <table class="table table-bordered table-hover table-checkable dataTable dtr-inline" <?php /*?>id="m_table_1"<?php */?>>
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td><a href="edit_order.php?order_id=<?=$order_data['order_id']?>&order_mode=paid"><?=$order_data['order_id']?></a></td>
                          ...
                          <td Width="150">
                            <?php /*?><a href="order_offer.php?order_id=<?=$order_data['order_id']?>&order_mode=paid" class="btn btn-accent m-btn m-btn--custom m-btn--pill m-btn--icon m-btn--air btn-xs">Offer</a><?php */?>
							<?php
							if($prms_order_edit == '1') { ?>
                            <a href="edit_order.php?order_id=<?=$order_data['order_id']?>&order_mode=paid" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill"><i class="la la-edit"></i></a>
							<?php
							}
							if($prms_order_delete == '1') { ?>
                            <a href="controllers/order/order.php?a_id=<?=$order_data['order_id']?>&order_mode=paid" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" onclick="return confirm('Are you sure you want to archive this record?')"><i class="la la-trash"></i></a>
							<?php
							} ?>
                          </td>
                        </tr>
...
jQuery(document).ready(function($) {
	$('.searchbx').on('click', function(e) {
		...
		if(val=="" && from_date=="" && to_date=="" && status=="" && payment_paid_batch_id=="") {
