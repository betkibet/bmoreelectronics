...
$order_p_query=mysqli_query($db,"SELECT COUNT(*) AS num_of_orders, o.order_id, u.first_name, u.last_name, ca.contractor_id FROM orders AS o LEFT JOIN users AS u ON u.id=o.user_id LEFT JOIN contractor_orders AS ca ON o.order_id=ca.order_id WHERE o.status!='partial' AND o.is_payment_sent='1' AND o.is_trash='0' ".$filter_by);
$order_p_data = mysqli_fetch_assoc($order_p_query);
$pages->set_total($order_p_data['num_of_orders']);

//Fetch list of order
$order_query=mysqli_query($db,"SELECT o.*, u.first_name, u.last_name, p.shop_name as aflt_shop_name, os.name AS order_status_name, c.name as contractor_name, ca.contractor_id FROM orders AS o LEFT JOIN users AS u ON u.id=o.user_id LEFT JOIN affiliate AS p ON p.id=o.affiliate_id LEFT JOIN order_status AS os ON os.id=o.status LEFT JOIN contractor_orders AS ca ON o.order_id=ca.order_id LEFT JOIN contractors AS c ON ca.contractor_id=c.id WHERE o.status!='partial' AND o.is_payment_sent='1' AND o.is_trash='0' ".$filter_by." ".$order_by." ".$pages->get_limit());

$order_paid_batch_q=mysqli_query($db,"SELECT o.*, u.first_name, u.last_name FROM orders AS o LEFT JOIN users AS u ON u.id=o.user_id WHERE o.status!='partial' AND o.is_payment_sent='1' AND o.is_trash='0' GROUP BY o.payment_paid_batch_id ORDER BY o.date DESC");
