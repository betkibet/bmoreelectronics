...
if(isset($post['d_id'])) {
	...
	if(!empty($exist_data)) {
		...
		$_SESSION['error_msg']=$msg;
		if($id) {
			setRedirect(ADMIN_URL.'edit_custom_page.php?id='.$id.($post['slug']?'&slug='.$post['slug']:''));
		} else {
			setRedirect(ADMIN_URL.'edit_custom_page.php');
		}
		exit();
	}
	...
	if($_FILES['image']['name']) {
		...
		if($image_ext=="png" || $image_ext=="jpg" || $image_ext=="jpeg" || $image_ext=="gif") {
			...
			$_SESSION['success_msg']=$msg;
			if($post['id']) {
				setRedirect(ADMIN_URL.'edit_custom_page.php?id='.$post['id']);
			} else {
				setRedirect(ADMIN_URL.'edit_custom_page.php');
			}
			exit();
